...
describe('Invoice Routes', () => {
    ...
    test('POST /invoices - create a new invoice', async () => {
        const newInvoice = { comp_code: 'testco', amt: 150 };
    ...
    test('GET /invoices/:id - fetch a single invoice by ID', async () => {
        ...
        const insertRes = await db.query(`INSERT INTO invoices (comp_code, amt) VALUES ('testco', 200) RETURNING id`);
    ...
    test('PUT /invoices/:id - update an invoice', async () => {
        ...
        const insertRes = await db.query(`INSERT INTO invoices (comp_code, amt) VALUES ('testco', 250) RETURNING id`);
        const invoiceId = insertRes.rows[0].id;
    ...
    test('DELETE /invoices/:id - delete an invoice', async () => {
        ...
        const insertRes = await db.query(`INSERT INTO invoices (comp_code, amt) VALUES ('testco', 100) RETURNING id`);
        const invoiceId = insertRes.rows[0].id;
